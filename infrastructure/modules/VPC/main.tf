...
resource "aws_route" "public_routes" {
  ...
  nat_gateway_id       = each.value.use_nat_gateway ? try(aws_nat_gateway.this[0].id, null) : null
  instance_id          = each.value.use_nat_instance ? try(aws_instance.nat_instance[0].id, null) : null
}
